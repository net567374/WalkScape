...
export default function ArtifactScanner() {
    ...
    const handleTouchGrass = async () => {
        ...
        try {
            ...
            const tx = await (contractWithSigner as unknown as { touchGrass: (locationHash: string) => Promise<{ hash: string; wait: () => Promise<unknown> }> }).touchGrass(locationHash);
    ...
    const handleClaimArtifact = async () => {
        ...
        try {
            ...
            const contractWithSigner = contract.connect(signer);
            // Use 'unknown' first as recommended by TypeScript
            const tx = await (contractWithSigner as unknown as { claimArtifact: (locationHash: string, artifactType: ArtifactType) => Promise<{ hash: string; wait: () => Promise<unknown> }> }).claimArtifact(locationHash, selectedArtifactType);
            console.log('Transaction submitted:', tx.hash);
